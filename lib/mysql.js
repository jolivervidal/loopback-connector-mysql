      ...
      if (relations && relations[key]) {
        ...
        if (type == 'belongsTo') {
          ...
          joinOn = throughModelAlias + '.' + childForeignKey +
            ' = ' + childTableAlias + '.id';            
        } else if (type == "hasAndBelongsToMany" && relation.throughTable) {
          const throughModel = this.tableEscaped(relation.throughTable);
          const throughModelAlias = getTableAlias(
            relation.throughTable,
            aliasCount
          );
          aliases[relation.throughTable] = throughModelAlias;
          let parentForeignKey =
            relation.foreignKey || lowerCaseFirstLetter(model) + "Id";
          let childForeignKey =
            relation.keyThrough || lowerCaseFirstLetter(relation.model) + "Id";
          joinThrough =
            `LEFT JOIN ${throughModel} ${throughModelAlias}
            ON ${throughModelAlias}.${parentForeignKey}` +
            ` = ${tableAlias}.id`;
          joinOn = throughModelAlias + '.' + childForeignKey +
          ' = ' + childTableAlias + '.id';
